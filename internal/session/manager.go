...
func cookieS(log *slog.Logger, cachePath string) ([][]Cookie, error) {
	cookiesPath := filepath.Join("data", "grok", "grok.com", "cookies")
	files, err := os.ReadDir(cookiesPath)
	if err != nil {
		return nil, fmt.Errorf("чтение директории куки: %w", err)
	}
	if len(files) == 0 {
		return nil, fmt.Errorf("папка пустая, добавьте JSON файлы с куками")
	}
